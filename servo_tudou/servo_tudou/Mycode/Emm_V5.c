...
Emm_struct Emm_motor[3];

/* 绝对值运动模式 ：一圈360度。初始为0，转260度后转-180度，即先反转260到零点，再反转180   */
void Emm_Init()
{
	Emm_motor[0].addr	=0x01;							//电机地址
	Emm_motor[0].acc =200;							//加速度
	Emm_motor[0].vel =3000;
	Emm_motor[0].angle =0;
	Emm_motor[0].raF =0;							//相位/绝对标志，false为相对运动，true为绝对值运动
	Emm_motor[0].snF =0;						//多机同步标志 ，false为不启用，true为启用
	
	Emm_motor[1].addr	=0x01;				
	Emm_motor[1].acc =200;				
	Emm_motor[1].vel =3000;
	Emm_motor[1].angle =0;
	Emm_motor[1].raF =0;					
	Emm_motor[1].snF =0;
	
	Emm_motor[2].addr	=0x01;				
	Emm_motor[2].acc =200;				
	Emm_motor[2].vel =3000;
	Emm_motor[2].angle =0;
	Emm_motor[2].raF =0;					
	Emm_motor[2].snF =0;
...
void Emm_V5_Pos_Control(UART_HandleTypeDef *huart,uint8_t addr, uint16_t vel, uint8_t acc, float angle, bool raF, bool snF)
{
  uint8_t cmd[16] = {0};
	uint8_t dir=0;
	uint32_t clk=0;
	...
	clk=(uint32_t)ABS(angle)*3200/360.0f;
